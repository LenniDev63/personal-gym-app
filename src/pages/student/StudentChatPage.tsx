...
interface Conv { id: string; trainer_id: string; name: string; lastMessage: string; time: string; }
...
export default function StudentChatPage() {
  ...
  const loadConversations = async () => {
    ...
    const { data: convs } = await supabase
      .from('conversations').select('id, trainer_id, last_message_at')
      .eq('student_id', user.id).order('last_message_at', { ascending: false });

    if (!convs || convs.length === 0) { setConversations([]); return; }

    const trainerIds = convs.map((c) => c.trainer_id);
    const { data: profs } = await supabase.from('profiles').select('id, full_name, email').in('id', trainerIds);
    ...
    setConversations(convs.map((c) => {
      const p = profMap.get(c.trainer_id);
      const m = lastByConv.get(c.id);
      return {
        id: c.id,
        trainer_id: c.trainer_id,
        name: p?.full_name || p?.email || 'Coach',
  ...
  // Realtime: refresh list when any new conversation or message arrives
  useEffect(() => {
    if (!user) return;
    const ch = supabase.channel(`student-msgs-${user.id}`)
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'conversations', filter: `student_id=eq.${user.id}` }, () => loadConversations())
      .subscribe();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const channel = supabase.channel(`smsgs-${selected.id}`)
    ...
    return (
      <div className="h-screen flex flex-col bg-background">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-32">
          {messages.length === 0 ? <p className="text-center text-muted-foreground text-sm">No messages yet. Send the first one.</p> :
            messages.map((m) => (
  ...
  return (
    <div className="px-4 py-6 safe-area-top">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm mt-1">Chat with your trainers</p>
      </div>
      <div className="space-y-3">
        {conversations.length === 0 ? <div className="fitness-card text-center py-12 text-muted-foreground">No conversations yet. A trainer will reach out once approved.</div> :
          conversations.map((c) => (
