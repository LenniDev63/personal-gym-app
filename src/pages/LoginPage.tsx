...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        // Try auto-login (works only if email confirmation is disabled)
  ...
  const handleRoleSelect = (r: UserRole) => { setSelectedRole(r); setMode('login'); };
  const handleBack = () => { if (mode !== 'select-role') setMode('select-role'); else navigate('/'); };

  return (
    <div className="min-h-screen bg-background flex flex-col px-6 py-8 safe-area-top safe-area-bottom">
      <motion.button initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} onClick={handleBack} className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors w-fit mb-8">
      ...
      <div className="flex-1 flex flex-col justify-center max-w-sm mx-auto w-full">
        ...
        <AnimatePresence mode="wait">
          {mode === 'select-role' && (
            <motion.div key="select-role" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-6">
              ...
              <button onClick={() => handleRoleSelect('trainer')} className="w-full fitness-card p-6 flex items-center gap-4 hover:shadow-card-hover transition-all group">
                <div className="w-14 h-14 bg-primary/10 rounded-xl flex items-center justify-center group-hover:bg-primary/20 transition-colors"><User className="w-7 h-7 text-primary" /></div>
                <div className="text-left flex-1"><h3 className="font-semibold text-foreground text-lg">Personal Trainer</h3><p className="text-muted-foreground text-sm">Manage students and workouts</p></div>
              </button>
              <button onClick={() => handleRoleSelect('student')} className="w-full fitness-card p-6 flex items-center gap-4 hover:shadow-card-hover transition-all group">
                <div className="w-14 h-14 bg-accent rounded-xl flex items-center justify-center group-hover:bg-primary/20 transition-colors"><GraduationCap className="w-7 h-7 text-primary" /></div>
                <div className="text-left flex-1"><h3 className="font-semibold text-foreground text-lg">Student</h3><p className="text-muted-foreground text-sm">Access your workouts and progress</p></div>
              </button>
            </motion.div>
          )}
          ...
          {(mode === 'login' || mode === 'register') && (
            <motion.div key="auth-form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-6">
              <div className="text-center mb-6">
                <div className="fitness-badge-primary mb-3 mx-auto w-fit">{selectedRole === 'trainer' ? 'Personal Trainer' : 'Student'}</div>
                <h1 className="text-2xl font-bold text-foreground mb-2">{mode === 'login' ? 'Welcome back!' : 'Create your account'}</h1>
                <p className="text-muted-foreground">{mode === 'login' ? 'Enter your credentials' : 'Fill in your details below'}</p>
              </div>
