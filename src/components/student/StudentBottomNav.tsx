...
const navItems = [
  { path: '/student', icon: Dumbbell, label: 'Workouts' },
        ...
        {navItems.map((item) => {
          ...
          return (
            <NavLink key={item.path} to={item.path} className={`fitness-nav-item relative flex-1 min-w-0 ${isActive ? 'active' : ''}`}>
              {isActive && <motion.div layoutId="student-nav-indicator" className="absolute -top-2 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-primary" transition={{ type: 'spring', bounce: 0.3, duration: 0.4 }} />}
